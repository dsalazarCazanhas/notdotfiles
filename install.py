...
def install_oh_my_zsh() -> bool:
    ...
    try:
        ...
        # Run installer
        success, _ = run_command(
            ["sh", str(install_script), "--unattended"], "Failed to install Oh My Zsh"
        )
...
def install_yay() -> bool:
    ...
    success, _ = run_command(
        ["makepkg", "-si", "--noconfirm"], "Failed to build yay-bin", cwd=build_dir
    )
    shutil.rmtree(build_dir, ignore_errors=True)
    return success


def already_updated_system() -> bool:
    ...
    try:
        ...
        if response not in ["y", "yes"]:
            logger.info("I see, please upgrade your system first.")
            sys.exit(0)
...
def install_packages() -> bool:
    ...
    # If we are here, then is cuz Yay is already installed
...
def copy_config_files() -> bool:
    ...
    configs = [
        ...
        (
            INSTALL_DIR / "config" / "alacritty.toml",
...
def prepare() -> bool:
    ...
    steps = [
        ("Updating system?NO", already_updated_system),
        ("Installing yay if not installed, I doubt it", install_yay),
